...
class ImageDataset(Dataset):
    def __init__(self):
        ...
        self.lr = torch.from_numpy(np.load("./train/CFD032_list.npy").astype(np.float32)).clone()
        self.hr = torch.from_numpy(np.load("./train/CFD128_list.npy").astype(np.float32)).clone()
