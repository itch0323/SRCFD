...
from datasets import * #datasets.pyを読み込む
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        # Adversarial loss
        loss_GAN = criterion_GAN(gen_hr, imgs_hr)
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            np.save("lr/%d" % batches_done, imgs_lr.to('cpu').detach().numpy().copy())
            np.save("hr/%d" % batches_done, imgs_hr.to('cpu').detach().numpy().copy())
            np.save("sr/%d" % batches_done, gen_hr.to('cpu').detach().numpy().copy())
...
torch.save(generator.state_dict(), "saved_model2/generator_%d.pth" % epoch)
